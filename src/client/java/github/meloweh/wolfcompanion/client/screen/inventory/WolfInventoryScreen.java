...
public class WolfInventoryScreen extends AbstractContainerScreen<WolfInventoryScreenHandler> {
    ...
    public boolean mouseClicked(MouseButtonEvent click, boolean doubled) {
        if (clickedDropChest(click.x(), click.y()) && wolfState().hasChestEquipped()) {
    ...
    private void drawReleaseButton(GuiGraphicsExtractor context, int mouseX, int mouseY) {
        ...
        if (isMouseOver(x, y, RELEASE_BUTTON_WIDTH, RELEASE_BUTTON_HEIGHT)) {
            context.blit(RenderPipelines.GUI_TEXTURED, WolfInventoryTextures.RELEASE_WOLF_HOVERED, x, y, 0, 0, RELEASE_BUTTON_WIDTH, RELEASE_BUTTON_HEIGHT, RELEASE_BUTTON_WIDTH, RELEASE_BUTTON_HEIGHT);
            context.setTooltipForNextFrame(this.font, Component.nullToEmpty("Release Wolf"), mouseX, mouseY);
        } else {
            context.blit(RenderPipelines.GUI_TEXTURED, WolfInventoryTextures.RELEASE_WOLF, x, y, 0, 0, RELEASE_BUTTON_WIDTH, RELEASE_BUTTON_HEIGHT, RELEASE_BUTTON_WIDTH, RELEASE_BUTTON_HEIGHT);
        }
    }

    private void drawAggressionButton(GuiGraphicsExtractor context, int mouseX, int mouseY) {
        if (!this.wolf.isTame()) return;

        int x = left() + CHEST_BUTTON_X;
        int y = top() + AGGRESSION_BUTTON_Y;

        final Identifier hoveredTexture = wolfState().isAggressive__() ? WolfInventoryTextures.OFFENSIVE_MODE_HOVERED : WolfInventoryTextures.VANILLA_MODE_HOVERED;
        final Identifier texture = wolfState().isAggressive__() ? WolfInventoryTextures.OFFENSIVE_MODE : WolfInventoryTextures.VANILLA_MODE;

        if (isMouseOver(x, y, STANDARD_BUTTON_SIZE, STANDARD_BUTTON_SIZE)) {
            context.blit(RenderPipelines.GUI_TEXTURED, hoveredTexture, x, y, 0, 0, STANDARD_BUTTON_SIZE, STANDARD_BUTTON_SIZE, STANDARD_BUTTON_SIZE, STANDARD_BUTTON_SIZE);
            context.setTooltipForNextFrame(this.font, Component.nullToEmpty("Toggle Aggression"), mouseX, mouseY);
        } else {
            context.blit(RenderPipelines.GUI_TEXTURED, texture, x, y, 0, 0, STANDARD_BUTTON_SIZE, STANDARD_BUTTON_SIZE, STANDARD_BUTTON_SIZE, STANDARD_BUTTON_SIZE);
        }
    }

    private void drawLockButton(GuiGraphicsExtractor context, int mouseX, int mouseY) {
        if (!this.wolf.isTame()) return;
        ...
        final Identifier hoveredTexture = wolfState().isLock__() ? WolfInventoryTextures.LOCKED_HOVERED : WolfInventoryTextures.UNLOCKED_HOVERED;
        final Identifier texture = wolfState().isLock__() ? WolfInventoryTextures.LOCKED : WolfInventoryTextures.UNLOCKED;

        if (isMouseOver(x, y, LOCK_BUTTON_WIDTH, height)) {
            context.blit(RenderPipelines.GUI_TEXTURED, hoveredTexture, x, y, 0, 0, LOCK_BUTTON_WIDTH, LOCK_BUTTON_HEIGHT, LOCK_BUTTON_WIDTH, LOCK_BUTTON_HEIGHT);
            context.setTooltipForNextFrame(this.font, Component.nullToEmpty("Resist Whistle"), mouseX, mouseY);
        } else {
            context.blit(RenderPipelines.GUI_TEXTURED, texture, x, y, 0, 0, LOCK_BUTTON_WIDTH, LOCK_BUTTON_HEIGHT, LOCK_BUTTON_WIDTH, LOCK_BUTTON_HEIGHT);
        }
    }

    private void drawChestButton(GuiGraphicsExtractor context, int mouseX, int mouseY) {
        if (!this.wolf.isTame()) return;
        ...
        if (isMouseOver(x, y, STANDARD_BUTTON_SIZE, STANDARD_BUTTON_SIZE)) {
            context.blit(RenderPipelines.GUI_TEXTURED, WolfInventoryTextures.CHEST_HOVERED, x, y, 0, 0, STANDARD_BUTTON_SIZE, STANDARD_BUTTON_SIZE, STANDARD_BUTTON_SIZE, STANDARD_BUTTON_SIZE);
            context.setTooltipForNextFrame(this.font, Component.nullToEmpty("Drop Bag and Items"), mouseX, mouseY);
        } else {
            context.blit(RenderPipelines.GUI_TEXTURED, WolfInventoryTextures.CHEST, x, y, 0, 0, STANDARD_BUTTON_SIZE, STANDARD_BUTTON_SIZE, STANDARD_BUTTON_SIZE, STANDARD_BUTTON_SIZE);
        }
    }
    ...
    public void extractBackground(GuiGraphicsExtractor context, int mouseX, int mouseY, float deltaTicks) {
        ...
        int i = left();
        int j = top();
        context.blit(RenderPipelines.GUI_TEXTURED, WolfInventoryTextures.BACKGROUND, i, j, 0.0F, 0.0F, this.imageWidth, this.imageHeight, 256, 256);

        if (wolfState().hasChestEquipped()) {
            context.blitSprite(RenderPipelines.GUI_TEXTURED, WolfInventoryTextures.CHEST_SLOTS, 90, 54, 0, 0, i + 79, j + 17, WOLF_SLOT_COLUMNS * 18, 54);
        }

        if (this.wolf.isWearingBodyArmor()) {
            context.blitSprite(RenderPipelines.GUI_TEXTURED, WolfInventoryTextures.SLOT, i + 7, j + 35 - 18, 18, 18);
        } else {
            context.blit(RenderPipelines.GUI_TEXTURED, WolfInventoryTextures.WOLF_ARMOR_SLOT, i + 7, j + 35 - 18, 0, 0, 18, 18, 18, 18);
        }
    ...
    private void drawHearts(GuiGraphicsExtractor context) {
        ...
        context.blit(RenderPipelines.GUI_TEXTURED, WolfInventoryTextures.HEART_CONTAINER, x, y, 0, 0, WIDTH * maxHealthPoints + 1, HEIGHT, WIDTH, HEIGHT);
        context.blit(RenderPipelines.GUI_TEXTURED, WolfInventoryTextures.HEART, x, y, 0, 0, healthPixels, HEIGHT, WIDTH, HEIGHT);
    }
    ...
    private void drawLevelInfo(GuiGraphicsExtractor context) {
        ...
        context.blit(RenderPipelines.GUI_TEXTURED, WolfInventoryTextures.XP_BACKGROUND, x, y, 0, 0, WIDTH, HEIGHT, WIDTH, HEIGHT);
        context.blit(RenderPipelines.GUI_TEXTURED, WolfInventoryTextures.XP_CURRENT, x, y, 0, 0, WIDTH * deltaXp / deltaMaxXp, HEIGHT, WIDTH, HEIGHT);
